...
local function orderedValues(raw)
  ...
  for _, entry in ipairs(numbered) do table.insert(values, entry.value) end
  for _, entry in ipairs(other) do table.insert(values, entry.value) end
  return values
end
...
function UI.build()
  ...
  if type(Geyser) ~= "table" or not Geyser.UserWindow or not Geyser.Mapper or not Geyser.MiniConsole then
    cecho("\n<red>[ZeroPoint UI] Geyser UserWindow/Mapper/MiniConsole support is required.<reset>\n")
    return false
  end
...
function UI.show()
  UI.visible = true
  if UI.build() then UI.window:show() end
end
...
function UI.status()
  cecho("\n<cyan>[ZeroPoint UI]<reset> v" .. UI.version .. " | " .. (UI.visible and "<green>visible" or "<red>hidden") .. "<reset>\n")
end
...
function UI.bootstrap()
  ...
  table.insert(UI.eventIDs, registerAnonymousEventHandler("msdp.ROOM", function()
    UI.onMSDPRoom()
  end))
